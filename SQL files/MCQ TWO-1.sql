...
SELECT DAYNAME(time_of_record) AS Day_of_the_week, ROUND(AVG(time_in_queue)) AS Avg_queue_time
FROM visits
WHERE time_in_queue != 0
GROUP BY DAYNAME(time_of_record);

-- Average queue for Hour of the day 
SELECT HOUR(time_of_record) AS Hour_of_the_day, ROUND(AVG(time_in_queue)) AS Avg_queue_time
FROM visits
WHERE time_in_queue != 0
GROUP BY Hour_of_the_day;

-- Average queue for Hour of the day 
SELECT TIME_FORMAT(time_of_record, "%H:00") AS Hour_of_the_day, ROUND(AVG(time_in_queue)) AS Avg_queue_time
FROM visits
WHERE time_in_queue != 0
GROUP BY Hour_of_the_day
ORDER BY Avg_queue_time DESC, Hour_of_the_day;

-- Time in queue for Hour of the day and day of the week
...
-- Average queue for Hour of the day and day of the week
SELECT TIME_FORMAT(TIME(time_of_record), '%H:00') AS hour_of_day,
-- Sunday
ROUND(AVG(CASE
			WHEN DAYNAME(time_of_record) = 'Sunday' THEN time_in_queue
